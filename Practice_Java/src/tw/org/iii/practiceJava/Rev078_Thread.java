 ...
 * 	=> 新執行緒
 * 	=> 代表一個任務
 * 	=> 通常一個執行緒只做一件獨立的事情 (完整的一件事)
 * 	=> 主緒往下執行, 遇到一個執行緒start, 是將其交到Run Pool中排隊等候執行.
 * 		CPU把時間做很細的切割 
 * 		每個時間點到的時候CPU在Run Pool中選擇一個執行緒來做, 時間到了換下一個 (讓人很像同步執行)
 * 		如範例的t1/t2 因沒有設定優先等級, 先執行的機率是一樣的.
 * 		在各時間點重複挑選到相同的機率很低
 * 		沒有sleep前, 看不出來同時執行, 因迴圈在其分割的周期內一下就跑完了
 * 	=> start 時間週期性不強 (sleep無法控制時間週期, 只能做延遲性)
 * 	=> 完整的一個任務執行, 做完後生命結束(物件還在, 是其表現的生命沒了), 無法再呼叫start, 但可以呼叫物件方法run.
 * 	=> 生命週期-->表現同步的效果
 * 	=> run 物件方法 -->就只是循序執行 (生命特徵表現在run方法中)
 * 	=> wait() : 所有object都有, for Thread (媽媽等小明去買醬油
 * 	=> notify() : 某執行緒完成後發出通知 (買回來跟媽媽說
 *	=> 提早結束執行緒: 不要用stop方法(unsafe), 而是透過邏輯方法中止(e.g. break)
 * 
 * 	Thread.sleep (static method)
 * 	=> 睡覺 (從Run Pool中被踢出) 
 * 	=> 當下的CPU就沒人佔用了, CPU會再挑下一個 (所以Sleep後才會發現迴圈交替執行了)
 * 	=> 有插斷例外拋出 (睡到一半被叫醒)
...
//		// 多跑幾次會發現OK有時會在最下面或兩個執行緒間
		...
		t1.start();	// 開始執行緒的生命週期 (Queue in Run Pool)
		t2.start();
